...
import (
	...
	"github.com/paddman/CherrySentinelAgent/src/CherrySentinel.Agent.CentOS6/internal/agent"
...
func main() {
	var (
		configPath      = flag.String("config", "/etc/cherrysentinel-agent/config.json", "configuration file")
	...
	if *showVersion {
		fmt.Printf("Cherry Sentinel CentOS 6 Agent %s\n", version)
		return
	}
	if *writeConfig != "" {
		cfg := config.Default()
		...
		if token == "" {
			token = os.Getenv("CHERRY_BOOTSTRAP_ENROLLMENT_TOKEN")
		}
		key := *apiKey
		if key == "" {
			key = os.Getenv("CHERRY_BOOTSTRAP_API_KEY")
		}
...
func newLogger(path string) (*log.Logger, io.Closer, error) {
	...
	return log.New(writer, "cherrysentinel-centos6 ", log.LstdFlags|log.LUTC|log.Lmicroseconds), f, nil
}
