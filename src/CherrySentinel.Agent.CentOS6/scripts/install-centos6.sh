#!/bin/bash
# Cherry Sentinel legacy agent installer for CentOS/RHEL 6 (SysV init).
set -euo pipefail

INSTALL_DIR=/opt/cherrysentinel/centos6-agent
CONFIG_DIR=/etc/cherrysentinel-agent
CONFIG_FILE=$CONFIG_DIR/config.json
STATE_DIR=/var/lib/cherrysentinel-agent
SPOOL_DIR=/var/spool/cherrysentinel-agent
LOG_DIR=/var/log/cherrysentinel
INIT_FILE=/etc/init.d/cherrysentinel-agent
LOGROTATE_FILE=/etc/logrotate.d/cherrysentinel-agent
SERVICE=cherrysentinel-agent
...
Required:
  --url URL                  Cherry Sentinel Central URL
...
if [ -z "$BINARY_SOURCE" ]; then
    arch="$(uname -m)"
    case "$arch" in
        x86_64|amd64) names="cherrysentinel-agent cherrysentinel-agent-centos6-amd64" ;;
        i386|i486|i586|i686) names="cherrysentinel-agent cherrysentinel-agent-centos6-386" ;;
...
install -m 0755 "$BINARY_SOURCE" "$INSTALL_DIR/cherrysentinel-agent"

INIT_SOURCE=""
for candidate in "$SCRIPT_DIR/cherrysentinel-agent.init" "$SCRIPT_DIR/../packaging/sysv/cherrysentinel-agent"; do
...
LOGROTATE_SOURCE=""
for candidate in "$SCRIPT_DIR/cherrysentinel-agent.logrotate" "$SCRIPT_DIR/../packaging/logrotate/cherrysentinel-agent"; do
...
if [ ! -f "$CONFIG_FILE" ] || [ "$FORCE_CONFIG" -eq 1 ]; then
    ...
    CHERRY_BOOTSTRAP_ENROLLMENT_TOKEN="$ENROLLMENT_TOKEN" \
    CHERRY_BOOTSTRAP_API_KEY="$API_KEY" \
        "$INSTALL_DIR/cherrysentinel-agent" "${args[@]}"
else
...
"$INSTALL_DIR/cherrysentinel-agent" -config "$CONFIG_FILE" -check-config
...
Cherry Sentinel CentOS 6 agent installed.
  Binary : $INSTALL_DIR/cherrysentinel-agent
  Config : $CONFIG_FILE
...
Commands:
  service cherrysentinel-agent status
  tail -f /var/log/cherrysentinel/centos6-agent.log
  cat /var/lib/cherrysentinel-agent/status.json
DONE
