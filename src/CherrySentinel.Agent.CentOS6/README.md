# Cherry Sentinel CentOS 6 Legacy Agent

A small, dependency-free Linux agent for **CentOS/RHEL 6** hosts that cannot run the repository's current .NET 10/systemd agent. It builds as a static Go binary, runs under SysV init, and sends telemetry to the existing Cherry Sentinel Central API.

This module is intentionally isolated from `CherrySentinel.Agent.Linux`; it does not replace or downgrade the modern Linux agent.
...
```bash
cd src/CherrySentinel.Agent.CentOS6
make test vet
make build-amd64
```
...
```bash
tar -xzf CherrySentinel-CentOS6-Agent-0.1.0-amd64.tar.gz
sudo ./install-centos6.sh \
  --url https://CENTRAL_IP:7443 \
  --enrollment-token 'CENTRAL_ENROLLMENT_TOKEN' \
  --ca-file /root/cherry-central-ca.pem
```
...
service cherrysentinel-agent restart
...
| Item | Path |
|---|---|
| Binary | `/opt/cherrysentinel/centos6-agent/cherrysentinel-agent` |
| Config | `/etc/cherrysentinel-agent/config.json` |
| API key and offsets | `/var/lib/cherrysentinel-agent/` |
| Offline spool | `/var/spool/cherrysentinel-agent/` |
| Log | `/var/log/cherrysentinel/centos6-agent.log` |
| Init script | `/etc/init.d/cherrysentinel-agent` |
...
python - <<'PY'
import socket
msg = '<134>Aug  5 12:00:00 centos6 sshd[999]: Cherry Sentinel syslog test'
...
```bash
/opt/cherrysentinel/centos6-agent/cherrysentinel-agent \
  -config /etc/cherrysentinel-agent/config.json \
  -check-config
```
...
```bash
service cherrysentinel-agent stop
/opt/cherrysentinel/centos6-agent/cherrysentinel-agent \
  -config /etc/cherrysentinel-agent/config.json \
  -once
service cherrysentinel-agent start
```
