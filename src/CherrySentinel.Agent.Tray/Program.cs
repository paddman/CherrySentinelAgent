...
{
    private const string MutexName = "Global\\CherrySentinel.Agent.Tray.SingleInstance";
    ...
    private static string ResolveInstallDir(string[] args)
    {
        ...
        if (File.Exists(Path.Combine(baseDir, "CherrySentinel.Agent.exe")) ||
        ...
        var candidates = new[]
        {
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Cherry Sentinel Agent"),
            Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Cherry Sentinel", "Agent"),
            baseDir
        };
