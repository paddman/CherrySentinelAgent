...
/// <summary>
/// Local CherrySentinelAgent Windows service: detect / install / start / stop / restart.
...
{
    public const string ServiceName = "CherrySentinelAgent";
    public const string DisplayName = "Cherry Sentinel Agent";
    ...
    public static string? GetServiceBinaryPath()
    {
        try
        {
            ...
            // BINARY_PATH_NAME   : "C:\...\CherrySentinel.Agent.exe"
    ...
    public static string? FindAgentExe(string installDir)
    {
        ...
        if (!string.IsNullOrWhiteSpace(installDir))
            candidates.Add(Path.Combine(installDir, "CherrySentinel.Agent.exe"));
        candidates.Add(Path.Combine(AppContext.BaseDirectory, "CherrySentinel.Agent.exe"));
        candidates.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles),
            "Cherry Sentinel Agent", "CherrySentinel.Agent.exe"));
        candidates.Add(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles),
            "Cherry Sentinel", "Agent", "CherrySentinel.Agent.exe"));
    ...
    public static bool TryRepairBrokenBinaryPath(string installDir, IWin32Window? owner, bool startAfter)
    {
        ...
        if (exe is null)
        {
            MessageBox.Show(owner,
                ...
                "Reinstall:\n  CherrySentinel-Agent-Setup-*.exe\n  or Full Setup.\n\n" +
    ...
    public static void Control(string action, string installDir, IWin32Window? owner = null)
    {
        if (!IsInstalled())
        {
            ...
            {
                MessageBox.Show(owner,
                    "Agent service is not installed and CherrySentinel.Agent.exe was not found.\n\n" +
                    "Install with:\n" +
                    "  CherrySentinel-Agent-Setup-*.exe\n" +
            ...
            var r = MessageBox.Show(owner,
                "Windows service \"CherrySentinelAgent\" is not registered on this PC.\n\n" +
                $"Found agent:\n{exe}\n\n" +
        ...
        catch (Exception ex)
        {
            ...
            MessageBox.Show(owner,
                ...
                "    C:\\Program Files\\Cherry Sentinel Agent\n" +
                "    C:\\Program Files\\Cherry Sentinel\\Agent",
                "Agent service",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error);
    ...
    public static bool RegisterService(string agentExe, string installDir, IWin32Window? owner, bool startAfter)
    {
        try
        {
            ...
            var dataDir = @"C:\ProgramData\CherrySentinel\Agent";
            ...
            try
            {
                ...
                var conn = @"C:\ProgramData\CherrySentinel\connection.json";
    ...
    private static bool RegisterWithSc(string agentExe, bool startAfter, IWin32Window? owner)
    {
        ...
        {
            MessageBox.Show(owner,
                $"sc create failed (exit {code}).\nRun tray as Administrator, or install CherrySentinel-Agent-Setup.",
                "Agent service", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }

        RunElevated("sc.exe", $"description {ServiceName} \"Cherry Sentinel Endpoint Security Monitoring Agent\"");
