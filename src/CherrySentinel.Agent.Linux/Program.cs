...
using CherrySentinel.Agent.Linux.Collectors;
using CherrySentinel.Agent.Linux.Detection;
using CherrySentinel.Agent.Linux.Response;
...
// Cherry Sentinel Linux Agent — metrics + multi-stack logs + allowlisted remediation.
...
internal sealed class LinuxAgentWorker : BackgroundService
{
        ...
        void MaybeAlert(string key, bool hit, Severity sev, string title, string desc)
        {
            ...
            lock (_bufGate)
            {
                ...
                _eventBuf.Add(new SecurityEventRecord
                {
                    ...
                    Channel = "metrics",
                    ProviderName = "CherrySentinel.Linux.Metrics",
                    EventId = 8000,
    ...
    private void WriteLocalStatus(HostMetrics m, string metricsLine)
    {
        try
        {
            var dir = Directory.Exists("/var/lib/cherrysentinel")
                ? "/var/lib/cherrysentinel"
    ...
    {
        http.DefaultRequestHeaders.Remove("X-Cherry-Api-Key");
        if (!string.IsNullOrWhiteSpace(_apiKey))
            http.DefaultRequestHeaders.TryAddWithoutValidation("X-Cherry-Api-Key", _apiKey);
    }
