...
public sealed class SyslogForwarder
{
    ...
    public async Task SendAlertAsync(DetectionAlert alert, string computerName, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var app = string.IsNullOrWhiteSpace(_options.SyslogAppName) ? "CherrySentinel" : _options.SyslogAppName;
            var msg =
                $"CherrySentinel-ALERT RuleId={alert.RuleId} Severity={alert.Severity} Title={Sanitize(alert.Title)} " +
