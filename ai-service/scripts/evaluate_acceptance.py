...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the WESSUWAN controlled acceptance suite")
    parser.add_argument(
        "--suite",
...
async def run_suite(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="wessuwan-eval-") as temp_dir:
        root = Path(temp_dir)
        settings = Settings(
            environment="test",
            data_dir=root,
