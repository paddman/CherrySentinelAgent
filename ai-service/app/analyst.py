...
class SentinelBrain:
    ...
    def _fallback_fields(
        ...
        summary_th = (
            f"Cherry Sentinel ตรวจพบ {risk.classification} จาก {source} ไปยัง {destination} "
            f"ด้วยคะแนนความเสี่ยง {risk.score}/100. "
            + ("หลักฐานเด่นคือ " + "; ".join(strongest) + "." if strongest else "ยังต้องเก็บหลักฐานเพิ่ม.")
        )
        summary_en = (
            f"Cherry Sentinel detected {risk.classification} from {source} to {destination} "
    ...
    def _sanitize_actions(
        ...
        for raw in value:
            ...
            reason = safe_text(raw.get("reason") or "Recommended by Sentinel Brain", 700)
    ...
    def _specialist_prompt(name: str) -> str:
        return f"""
You are the {name} specialist inside Cherry Sentinel Brain, a defensive SOC platform.
...
""".strip()
    ...
    def _commander_prompt() -> str:
        return f"""
You are Cherry Sentinel Brain, the incident commander for a sovereign defensive AI-XDR.
