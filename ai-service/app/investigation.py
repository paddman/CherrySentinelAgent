...
class InvestigationOrchestrator:
            ...
            try:
                if planned.tool == "ioc_lookup":
                    ...
                    if not request.include_central_related or not tenant.central_url:
                        output = _ToolOutput([], [], "Cherry Central is not configured for this tenant.")
                        trace.append(
                            ToolCallTrace(
                                step=index,
                                tool=planned.tool,
    ...
    async def _plan(
        ...
        if request.include_central_related and tenant.central_url:
            defaults.append(PlannedTool(tool="central_related", reason="Find correlated incidents in Cherry Central."))
...
""".strip(),
    ...
    async def _tool_central_related(
            ...
            evidence.append(
                {
                    ...
                    "summary": (
                        f"Related Cherry Central incident {item.incident_id}: {item.title}; "
        ...
        return _ToolOutput(
            evidence=evidence,
            refs=refs,
            summary=f"Found {len(evidence)} related incidents in the tenant's Cherry Central.",
            count=len(evidence),
        )
