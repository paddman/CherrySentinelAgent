...
logger = logging.getLogger("cherry.sentinel.brain")
...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "Sentinel Brain started tenants=%s model=%s mode=%s",
...
app = FastAPI(
    title="Cherry Sentinel Brain",
    version=__version__,
    description=(
        "Evidence-grounded, multi-tenant AI SOC analyst for Cherry Sentinel, "
...
if settings.cors_origin_list:
    app.add_middleware(
        ...
        allow_headers=["Content-Type", "X-Cherry-Tenant", "X-Cherry-Api-Key"],
    )
