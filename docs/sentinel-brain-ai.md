# Cherry Sentinel Brain: Sovereign AI-XDR and SOC-as-a-Service

## Product position

Cherry Sentinel Brain is the AI intelligence layer above the existing security stack:

- Cherry Sentinel Windows/Linux Agent for endpoint events, processes, services, metrics and response
- Cherry Sentinel Central for enrollment, policy, correlation, incidents, action queue and audit
...
flowchart TB
  subgraph SENSOR[Telemetry and sensors]
    ENDPOINT[Cherry Endpoint Agents]
  ...
  subgraph CONTROL[Cherry control plane]
    CENTRAL[Cherry Sentinel Central]
    INCIDENT[XDR Incident Correlator]
    ACTION[Human-approved Action Queue]
    AUDIT[Audit Trail]
  end

  subgraph BRAIN[Sentinel Brain AIaaS]
...
When `CHERRY_ANALYSIS_MODE=multi_agent`, three specialist calls run concurrently:

| Specialist | Primary evidence |
|---|---|
| Network | Suricata, Zeek, network threat intelligence |
| Endpoint | Cherry incident, Windows events, process/service context, deception |
...
The existing Cherry Central action queue remains the enforcement boundary.
...
1. **Sentinel Brain verdict**: classification, risk, confidence, model/fallback state.
...
## Integration with Cherry Central

No .NET change is required for the first demo. Configure each AI tenant with its Cherry Central URL and operator API key, then call:

```http
POST /v1/central/incidents/{incident_id}/analyze
```

Sentinel Brain fetches `/api/v1/incidents/{id}`, analyzes it and stores the decision under the same tenant. This is intentionally server-configured rather than caller-supplied to prevent arbitrary URL fetching.
...
Cherry Central should remain responsible for RBAC, action execution and durable operator audit.
...
Sentinel Brain includes a defensive token registry for canary credentials, honey files, decoy API keys, decoy URLs and shares. The raw secret is returned only at creation. The database stores only its SHA-256 hash.
...
- its own API key
- Cherry Central URL and operator key
...
5. Cherry Agent identifies host, user, process, PID and service.
6. Existing XDR correlation creates one incident.
7. Sentinel Brain runs deterministic risk, ML baseline, playbook retrieval and specialists.
...
10. Operator approves containment; Cherry Central executes and audits it.
