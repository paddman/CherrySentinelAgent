# WESSUWAN Implementation Readiness
...
|---|---|
| Windows/Linux endpoint telemetry and response framework | Cherry Sentinel Agent, Central and installers |
| Cross-host authentication and process/service attribution | Central correlator and incident model |
| Suricata/Zeek/ASM evidence ingestion contract | Sentinel Brain unified incident payload |
...
3. related incident lookup from that tenant's server-configured Cherry Central
4. local defensive playbook search

The planner cannot add tools, repeat them indefinitely or exceed the configured step/time limit. It has no shell, arbitrary URL, scanner, firewall, account, process or service mutation capability. Final response actions remain recommendations behind Cherry Central operator approval.
...
The remaining Sentinel Brain endpoints are unchanged.
