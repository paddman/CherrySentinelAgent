...
1. **Central** on the server: `CherrySentinel-Central-Setup-*.exe`  
2. Note LAN IP from desktop **CONNECTION.txt** / `C:\ProgramData\CherrySentinel\connection.json`  
...
`C:\ProgramData\CherrySentinel\connection.json`
...
```powershell
# Admin
cd "C:\Program Files\Cherry Sentinel Agent\Installer"
...
```powershell
New-NetFirewallRule -DisplayName 'Cherry Sentinel Central' -Direction Inbound -Action Allow -Protocol TCP -LocalPort 7443
```
