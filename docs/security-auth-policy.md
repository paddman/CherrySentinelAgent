...
| **Agent** | Per-agent `ApiKey` (issued on register) | `X-Cherry-Api-Key` on heartbeat/ingest |
| **Operator** (Dashboard) | `OperatorApiKey` | `X-Cherry-Api-Key` on fleet/actions/policy |
...
`C:\ProgramData\CherrySentinel\Server\secrets.json`
...
"Security": {
  ...
  "SecretsFilePath": "C:\\ProgramData\\CherrySentinel\\Server\\secrets.json",
...
2. Start agent service — it registers, receives `AgentApiKey`, stores:
   - `C:\ProgramData\CherrySentinel\Agent\agent.credential`
   - and `Server:ApiKey` in appsettings when writable
3. Heartbeat/ingest use `X-Cherry-Api-Key`
