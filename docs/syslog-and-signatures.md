...
```text
<134>1 2026-... HOST CherrySentinel - - - CherrySentinel-ALERT RuleId=... Severity=High ...
```
...
```text
C:\ProgramData\CherrySentinel\Server\signatures\opensource-signatures.json
...
Restart `CherrySentinelCentral` (or wait for file reload on next match cycle).

### Firewall for syslog

```powershell
New-NetFirewallRule -DisplayName "Cherry Syslog UDP 5514" -Direction Inbound -Protocol UDP -LocalPort 5514 -Action Allow
```
