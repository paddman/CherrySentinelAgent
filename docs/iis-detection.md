...
Cherry Sentinel can monitor **IIS / ASP.NET** hosts using Event Log channels + process/network rules.
...
1. **Cherry Sentinel Agent** installed and pointed at Central.  
...
Get-Content "C:\Program Files\Cherry Sentinel Agent\config\rules.json" | Select-String "IIS_"

# Restart agent after rule update
Restart-Service CherrySentinelAgent

# Logs
Get-Content C:\ProgramData\CherrySentinel\Agent\logs\agent-*.log -Tail 50 |
