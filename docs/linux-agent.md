# Cherry Sentinel — Linux Agent (full)
...
- `Platform` = `linux`
- Local file: `/var/lib/cherrysentinel/status.json` (or `./data/status.json`)
...
| `CollectDiagnostics` | write diag file under `/var/log/cherrysentinel` |
...
cat /var/lib/cherrysentinel/status.json
```
