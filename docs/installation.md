...
```text
  ...
  CherrySentinel.Agent  --HTTPS-->  CherrySentinel.Server  <--  Dashboard (WPF)
...
| รายการ | ค่า |
|--------|-----|
| Service | `CherrySentinelCentral` |
| Install | `C:\Program Files\Cherry Sentinel Central` |
| Data / DB | `C:\ProgramData\CherrySentinel\Server\central.db` (SQLite) |
| URL | `https://localhost:7443` หรือ `https://<host>:7443` |

Silent:

```powershell
.\artifacts\setup\CherrySentinel-Central-Setup-1.0.0.exe /VERYSILENT /Port=7443
```
...
```text
artifacts\setup\CherrySentinel-Agent-Setup-1.0.5.exe
```
...
3. เลือกโฟลเดอร์ (ค่าเริ่มต้น `C:\Program Files\Cherry Sentinel Agent`)
4. ติ๊ก Start service หลังติดตั้ง
5. Finish

ตัวติดตั้งจะ:
- วางไฟล์ Agent + **tray icon** (`CherrySentinel.Agent.Tray.exe`)
- เขียน `Server.Url` ใน `appsettings.json`
- ลงทะเบียน Windows Service ชื่อ **`CherrySentinelAgent`**
...
| รายการ | ค่า |
|--------|-----|
| โปรเซส | `CherrySentinel.Agent.Tray.exe` |
...
Start Menu → **Cherry Sentinel Agent → Reconfigure Central URL**  
หรือ:

```powershell
# Elevated
& "${env:ProgramFiles}\Cherry Sentinel Agent\Installer\set-agent-central-url.ps1" `
  -CentralUrl https://10.0.0.5:7443
```
...
แล้ว `Restart-Service CherrySentinelAgent`

| รายการ | ค่า |
|--------|-----|
| Service | `CherrySentinelAgent` |
| Install | `C:\Program Files\Cherry Sentinel Agent` |
...
1. เปิด **Cherry Sentinel Dashboard**
...
| Agent service | `Get-Service CherrySentinelAgent` |
