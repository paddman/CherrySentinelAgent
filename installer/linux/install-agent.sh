#!/usr/bin/env bash
# Cherry Sentinel Linux Agent installer
# Usage:
...
for d in \
  ...
  "$(pwd)"
do
  if [[ -f "$d/CherrySentinel.Agent.Linux" || -f "$d/CherrySentinel.Agent.Linux.dll" ]]; then
    BIN_SRC="$d"
    break
  fi
done
...
if [[ -z "$BIN_SRC" ]]; then
  red "Cannot find CherrySentinel.Agent.Linux binaries next to this script."
...
if [[ -z "$CENTRAL_URL" && -z "$CENTRAL_HOST" ]]; then
  cyan "=== Cherry Sentinel Linux Agent ==="
...
EXE="$INSTALL_ROOT/CherrySentinel.Agent.Linux"
if [[ -f "$EXE" ]]; then
  chmod +x "$EXE"
elif [[ -f "$INSTALL_ROOT/CherrySentinel.Agent.Linux.dll" ]]; then
...
#!/usr/bin/env bash
cd "$(dirname "$0")"
exec dotnet CherrySentinel.Agent.Linux.dll "$@"
WRAP
  chmod +x "$EXE"
else
...
# systemd unit
UNIT_SRC="$INSTALL_ROOT/cherrysentinel-agent.service"
if [[ -f "$UNIT_SRC" ]]; then
  cp "$UNIT_SRC" "$SERVICE_FILE"
else
  cat > "$SERVICE_FILE" <<EOF
[Unit]
Description=Cherry Sentinel Linux Agent
After=network-online.target
Wants=network-online.target

[Service]
Type=notify
ExecStart=$INSTALL_ROOT/CherrySentinel.Agent.Linux
...
sed -i "s|ExecStart=.*|ExecStart=$INSTALL_ROOT/CherrySentinel.Agent.Linux|" "$SERVICE_FILE" || true
...
green "========================================"
green " Cherry Sentinel Linux Agent installed"
green "========================================"
