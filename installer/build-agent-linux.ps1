...
Write-Host " Cherry Sentinel - Linux Agent package" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
if (-not $SkipPublish) {
    ...
    $proj = Join-Path $Root "src\CherrySentinel.Agent.Linux\CherrySentinel.Agent.Linux.csproj"
    dotnet publish $proj `
        -c $Configuration `
        -r $Runtime `
        --self-contained true `
    ...
    Copy-Item (Join-Path $Root "src\CherrySentinel.Agent.Linux\appsettings.json") $publishDir -Force
    Copy-Item (Join-Path $Root "src\CherrySentinel.Agent.Linux\cherrysentinel-agent.service") $publishDir -Force
} else {
    Write-Host "[skip] Publish" -ForegroundColor DarkYellow
    if (-not (Test-Path (Join-Path $publishDir "CherrySentinel.Agent.Linux")) -and
        -not (Test-Path (Join-Path $publishDir "CherrySentinel.Agent.Linux.dll"))) {
