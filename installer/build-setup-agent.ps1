...
<#
.SYNOPSIS
  Publish Agent + Tray and build CherrySentinel-Agent-Setup-x.y.z.exe (Inno Setup).
...
Write-Host " Cherry Sentinel - Agent Setup.exe" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
$agentExe = Join-Path $agentOut "CherrySentinel.Agent.exe"
$trayExe = Join-Path $agentOut "CherrySentinel.Agent.Tray.exe"
...
$iss = Join-Path $Root "installer\CherrySentinel-Agent.iss"
...
$setup = Get-ChildItem $outDir -Filter "CherrySentinel-Agent-Setup-*.exe" |
