...
<#
.SYNOPSIS
  Install Cherry Sentinel Central Server as a Windows Service (default SQLite, port 7443).
...
param(
    ...
    [string]$InstallDir = "C:\Program Files\Cherry Sentinel Central",
...
if (-not (Test-Path (Join-Path $SourceDir "CherrySentinel.Server.exe"))) {
...
$exeSrc = Join-Path $SourceDir "CherrySentinel.Server.exe"
...
New-Service -Name $ServiceName `
    ...
    -Description "Cherry Sentinel Central API (agents + dashboard connect here)" `
