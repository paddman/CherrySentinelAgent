...
<#
.SYNOPSIS
  Installs Cherry Sentinel Dashboard (native WPF app) with icon shortcuts.
#>
[CmdletBinding()]
param(
    [string]$SourceDir = ".\artifacts\dashboard-win-x64",
    [string]$InstallDir = "C:\Program Files\Cherry Sentinel\Dashboard",
    [string]$IconSource = ".\assets\icons\CherrySentinel.ico",
    [string]$ServerUrl = "https://localhost:7443",
    [switch]$StartAfterInstall
)

$ErrorActionPreference = "Stop"
$ProductName = "Cherry Sentinel Dashboard"
$ExeName = "CherrySentinel.Dashboard.exe"
...
} elseif (Test-Path (Join-Path $SourceDir "Assets\CherrySentinel.ico")) {
    Copy-Item (Join-Path $SourceDir "Assets\CherrySentinel.ico") $iconDest -Force
...
$startMenu = Join-Path $env:ProgramData "Microsoft\Windows\Start Menu\Programs\Cherry Sentinel"
...
`$startMenu = '$startMenu'
Stop-Process -Name 'CherrySentinel.Dashboard' -Force -ErrorAction SilentlyContinue
