...
<#
.SYNOPSIS
  Regenerate Cherry Sentinel Central HTTPS certificate with public/LAN SANs.
...
param(
    ...
    [string]$InstallDir = "",
    [string]$DataDir = "C:\ProgramData\CherrySentinel\Server",
...
function Resolve-InstallDir {
    ...
    $candidates += @(
        "${env:ProgramFiles}\Cherry Sentinel Central",
        "${env:ProgramFiles}\Cherry Sentinel\Central",
        "${env:ProgramFiles}\CherrySentinel\Central"
    )
    foreach ($c in $candidates) {
        if ($c -and (Test-Path (Join-Path $c "CherrySentinel.Server.exe"))) {
            return (Resolve-Path $c).Path
        }
    }
    throw "CherrySentinel.Server.exe not found. Pass -InstallDir."
}
...
$cfg.Security | Add-Member -NotePropertyName CertificatePassword -NotePropertyValue "CherrySentinel!" -Force
