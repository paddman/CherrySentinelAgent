...
$exe = Join-Path $InstallDir "CherrySentinel.Agent.Tray.exe"
if (-not (Test-Path $exe)) {
    $alt = Get-ChildItem -Path $InstallDir -Filter "CherrySentinel.Agent.Tray.exe" -Recurse -ErrorAction SilentlyContinue |
...
try {
    ...
    if (Test-Path $startup) {
        $lnkPath = Join-Path $startup "Cherry Sentinel Agent Tray.lnk"
        ...
        $s.WindowStyle = 7
        $s.Description = "Cherry Sentinel Agent system tray icon"
        $ico = Join-Path $InstallDir "CherrySentinel.ico"
