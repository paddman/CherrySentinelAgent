...
if (-not (Test-Path $exe)) {
    ...
    $fallbacks = @(
        (Join-Path ${env:ProgramFiles} "Cherry Sentinel Agent\CherrySentinel.Agent.exe"),
        (Join-Path ${env:ProgramFiles} "Cherry Sentinel\Agent\CherrySentinel.Agent.exe")
    )
...
$connPath = "C:\ProgramData\CherrySentinel\connection.json"
if (Test-Path $connPath) {
    try {
        ...
        if ($isLocal -and $conn.RemoteUrl) {
            ...
            $centralSvc = Get-Service -Name "CherrySentinelCentral" -ErrorAction SilentlyContinue
            if (-not $centralSvc) {
...
$legacy = Join-Path $env:ProgramFiles "Cherry Sentinel\Agent\CherrySentinel.Agent.exe"
